...
async function scrapematch(match) {
  ...
  try {
    await page.goto(`${match}&Print=Y`);
    // match details
    const ScorecardHeader = await page.$("td.ScorecardHeader");
    if (ScorecardHeader) {
      const __matchName = await (
        await ScorecardHeader.getProperty("innerText")
      ).jsonValue();
      if (__matchName) {
        matchName.directory = __matchName.split(" - ")[0];
        matchName.fileName = __matchName.split(" - ")[1];
      }
    }
    ...
    if (!fs.existsSync(matchName.directory)) {
      fs.mkdirSync(matchName.directory, { recursive: true });
      console.log(`Directory '${matchName.directory}' created successfully.`);
    } else {
      console.log(`Directory '${matchName.directory}' already exists.`);
    }
    const filePath = path.join(
      matchName.directory,
      `${matchName.fileName}.json`
    );
    fs.writeFile(
      filePath,
